  ...
  return (
    <section id="faq" aria-labelledby="faq-title" className="scroll-mt-24 border-t border-neutral-200 bg-neutral-50 px-6 py-10 sm:px-10 sm:py-12 lg:py-20 xl:py-24 lg:px-16">
      <div className="mx-auto max-w-6xl">
        ...
        {!expanded && (
          <div className="mt-6 text-center lg:mt-3 lg:pl-8 lg:text-left">
            <button type="button" aria-expanded={false} aria-controls="faq-questions" onClick={() => setExpanded(true)} className="rounded-2xl sm:rounded-full max-lg:action-gradient-outline px-7 py-2.5 sm:py-3 text-base font-medium text-brand-700 transition max-lg:hover:shadow-md lg:rounded-none lg:border-0 lg:bg-transparent lg:px-0 lg:text-neutral-600 lg:hover:text-neutral-800 lg:hover:underline lg:underline-offset-4">
        ...
        <div className="mx-auto mt-8 w-full max-w-lg rounded-3xl p-6 text-center sm:p-8" style={{ background: "radial-gradient(ellipse at center, #f8fbff 0%, #e0e7ff 55%, #c4b5fd 100%)" }}>
          ...
          <button type="button" onClick={() => openPanel("Consultation")} className="mt-5 inline-flex items-center justify-center gap-3 rounded-2xl sm:rounded-full max-lg:action-gradient-outline px-6 py-2.5 sm:py-4 text-base font-semibold text-brand-700 transition hover:shadow-md lg:border-2 lg:border-gray-400 lg:bg-white lg:hover:bg-brand-50">
