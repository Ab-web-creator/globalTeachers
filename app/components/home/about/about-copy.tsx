...
export default function AboutCopy({ openPanel }: PanelProps) {
  return (
    <div data-reveal className="px-6 py-2 sm:px-0 lg:py-4">
      ...
      <AboutBenefits />
      <button onClick={() => openPanel("Courses")} className="mt-6 rounded-full action-gradient-outline px-8 py-4 text-base font-medium text-brand-700 transition hover:shadow-md sm:mt-8">
        Начать свой путь
      </button>
    </div>
  );
