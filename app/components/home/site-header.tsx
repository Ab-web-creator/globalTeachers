...
export default function SiteHeader() {
  ...
  return (
    <header ref={headerRef} inert={hideHeader} aria-hidden={hideHeader} style={{ backgroundImage: !compact && !hidden ? "linear-gradient(to right, #2563eb, #7c3aed)" : undefined }} className={`fixed inset-x-0 top-0 z-40 transition-opacity duration-200 motion-reduce:transition-none ${compact ? "bg-white text-brand-700 shadow-sm" : "bg-transparent text-white"} ${hideHeader ? "pointer-events-none opacity-0" : "opacity-100"}`}>
